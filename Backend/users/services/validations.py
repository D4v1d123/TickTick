...
def username_is_valid(email):
    valid_email_structure = (
        r"^(?!.*\.\.)[^\s@]+@ticktick\.com$"  # username@domain.extension
    )
    invalid_characters = r"[\s,;&=_'\-\[\]+<>≤≥]"
    starts = r"^[@.]"  # Starts with @ or .
    ends = r"[@.]$"  # Ends with @ or .

    if not (re.fullmatch(valid_email_structure, email)) or re.findall(
        invalid_characters, email
    ):
        return False
...
def validate_field_data(request):
    ...
    if first_name and not (field_is_safe(first_name, r"^[a-zA-Z\s]+$")):
        errors["first_name"] = [
            "This field can only contain uppercase and lowercase letters."
        ]
    ...
    if gender and not (field_is_safe(gender, r"^[a-zA-Z\s-]+$")):
        errors["gender"] = [
            "This field can only contain uppercase, lowercase letters and hyphens."
        ]
...
def validate_img(file):
    ...
    if not (file_type in TYPE_IMAGES):
        return {
            "error": "Unsupported image format, use 'jpeg',"
            " 'png', 'webp', 'svg', 'heic' or 'heif'."
        }
